...
namespace YilanOyunu
{
    public partial class Form1 : MetroForm
    {
        int satirSayisi = 20;
        int sutunSayisi = 20;
        int bogumBoyut; //yılanın bir karesinin boyutu
        List<Point> yilan;
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            return base.ProcessCmdKey(ref msg, keyData); //Zıt yöndeki tuşa bastığında engel olup, En son tuşu algılayan
        }
        ...
        {
            BogumCiz(g, yilan[0].X, yilan[0].Y, true); // BAŞ true olduğu için ilk boğum gri oldu daha                                            sonra bogum çiz metodunda basmi default                                                 false olduğu için diğer boğumlar beyaz oldu.
        ...
        {
            int x = sutunNo * bogumBoyut; // bogum boyut => bir karenin boyutu 
        ...
        {
            Form2 frmSkorlar = new Form2();
            frmSkorlar.ShowDialog();
        }

        private void btnOyunuBaslat_Click(object sender, EventArgs e)
        {
            string ad = txtAdınız.Text.Replace(';', ' ').Trim(); ; ;
            if (ad == "")
            {
        ...
        {
            Point bas = yilan[0];
            Point yeniBas = kolayMi ? new Point((bas.X + 1 * xYon + sutunSayisi) % sutunSayisi, (bas.Y + 1 * yYon + satirSayisi) % satirSayisi) : new Point(bas.X + 1 * xYon, bas.Y + 1 * yYon);
                ...
                || yeniBas.Y < 0 || yeniBas.Y >= satirSayisi)
            {
                SkorKaydet();
                timer1.Stop();
                lblGameOver.Text = string.Format("OYUN BİTTİ \r\n SKORUNUZ: {0:000}\r\nTEKRAR OYNA(F2)", puan);
                lblGameOver.Show();
                oyunBittiMi = true;
                return;
            }
